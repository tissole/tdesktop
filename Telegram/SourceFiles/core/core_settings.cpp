...
QByteArray Settings::serialize() const {
	...
	size += sizeof(qint32) // _audioPlaybackSpeed
		+ sizeof(qint32) // _forwardOptions
		+ sizeof(qint32) // _groupingOptions
		+ sizeof(qint32) // _mediaGridZoomStep
		+ sizeof(qint32) // _pullToNextChannel
		+ sizeof(qint32) // _chatFiltersTabsMode
		+ sizeof(qint32) // _forwardOptionsEverSet
		+ sizeof(qint32); // _groupingOptionsEverSet
	...
	result.reserve(size);
	{
		...
		stream << qint32(_forwardOptions)
			...
			<< qint32(_mediaGridZoomStep)
			<< qint32(_pullToNextChannel.current() ? 1 : 0)
			<< qint32(_chatFiltersTabsMode.current())
...
void Settings::addFromSerialized(const QByteArray &serialized) {
	...
	case Data::ForwardOptions::PreserveInfo:
...
std::optional<int> Settings::readPrefImpl<int>(std::string_view key) {
	if (const auto data = readPrefGeneric(key)) {
		auto ok = false;
		if (const auto result = data->toInt(&ok); ok) {
			return result;
		}
	}
	return {};
