...
bool SkipMessageByDate(const Message &message, const Settings &settings) {
	...
	// Content & selection filters (export categories).
	...
	// Media constraint filters (size & extension), only for actual media files.
	...
	if (hasMediaFile) {
		if (!isSticker && settings.media.sizeLimit > 0
			&& message.file().size > settings.media.sizeLimit) {
			return true;
		}
		...
		if (settings.media.extensionFilterMode != MediaSettings::ExtFilterMode::None
			...
			QString name;
			QString ext;
			const bool isEligibleType = (type == Type::Video
				|| type == Type::Audio
				|| type == Type::File
				|| isSticker);
			if (isEligibleType) {
