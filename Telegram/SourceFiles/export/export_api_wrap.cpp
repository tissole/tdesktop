...
constexpr auto kMinRequestIntervalMs = 1000 / 41;
...
constexpr auto kThrottlerBurstCap =;
...
int GetConcurrentChunksForFile(int64 fileSize) {
	if (fileSize > 750 * kMegabyte) {
		return 15;  // Very large: maximum pipeline depth
	} else if (fileSize > 375 * kMegabyte) {
		return 15;  // Large
	} else if (fileSize > 32 * kMegabyte) {
		return 15;  // Medium
	}
	return 7;      // Small files: 2 concurrent chunks
}
