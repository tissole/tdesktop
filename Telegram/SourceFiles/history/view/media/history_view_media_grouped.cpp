...
QSize GroupedMedia::countOptimalSize() {
	...
	for (auto i = 0, count = int(layout.size()); i != count; ++i) {
		...
		if (GetEnhancedBool("caption_from_file_name") && !part.item->originalText().isEmpty()) {
			Ui::Text::String caption(st::messageTextStyle, part.item->originalText(), kDefaultTextOptions);
			const auto padding = QMargins(8, 4, 8, 4);
			part._captionHeight = caption.height(part.initialGeometry.width() - padding.left() - padding.right()) + padding.top() + padding.bottom();
		} else {
	...
	if (_mode == Mode::Grid && newWidth < st::historyGroupWidthMin) {
		...
		for (auto &part : _parts) {
			...
			if (GetEnhancedBool("caption_from_file_name") && !part.item->originalText().isEmpty()) {
				Ui::Text::String caption(st::messageTextStyle, part.item->originalText(), kDefaultTextOptions);
				const auto padding = QMargins(8, 4, 8, 4);
				part._captionHeight = caption.height(part.geometry.width() - padding.left() - padding.right()) + padding.top() + padding.bottom();
			} else {
				part._captionHeight = 0;
			}
