		...
		if (_peer->id == peerId) {
			...
			if (!canShowNow) {
				if (!_firstLoadRequest) {
					...
					jumpToMessage(showAtMsgId, params);
...
void HistoryWidget::clearDelayedShowAtRequest() {
	...
	if (_jumpToMessageRequest) {
		_history->owner().histories().cancelRequest(_jumpToMessageRequest);
		_jumpToMessageRequest = 0;
	}
}
...
void HistoryWidget::messagesFailed(const MTP::Error &error, int requestId) {
	...
	if (_preloadRequest == requestId) {
		...
		_firstLoadRequest = 0;
		closeCurrent();
	} else if (_jumpToMessageRequest == requestId) {
		_jumpToMessageRequest = 0;
	}
}
...
void HistoryWidget::messagesReceived(
	...
	case mtpc_messages_messages: {
		...
		count = histList->size();
		{
			...
			for (const auto &msg : *histList) {
				...
				totalCount++;
				if (msg.c_message().vmedia()) mediaCount++;
	...
	case mtpc_messages_channelMessages: {
		...
		count = d.vcount().v;
		{
			...
			for (const auto &msg : *histList) {
				if (msg.type() != mtpc_message) continue;
				totalCount++;
				if (msg.c_message().vmedia()) {
					mediaCount++;
					if (firstMediaInfo == u"none"_q) {
						const auto &media = *msg.c_message().vmedia();
						if (media.type() == mtpc_messageMediaPhoto) firstMediaInfo = u"photo"_q;
						else if (media.type() == mtpc_messageMediaDocument) firstMediaInfo = u"document"_q;
						else if (media.type() == mtpc_messageMediaWebPage) firstMediaInfo = u"webpage"_q;
						else firstMediaInfo = u"other:%1"_q.arg(media.type());
					}
				}
				if (msg.c_message().vrestriction_reason()) restCount++;
			}
	...
	if (_preloadRequest == requestId) {
		...
		_history->getReadyFor(jumpToMessageId);
		if (_history->isEmpty()) {
			addMessagesToFront(peer, *histList);
		}
		_firstLoadRequest = 0;
		...
		const auto skipId = (_migrated && jumpToMessageId < 0)
			? FullMsgId(_migrated->peer->id, -jumpToMessageId)
			: (jumpToMessageId > 0)
			? FullMsgId(_history->peer->id, jumpToMessageId)
			: FullMsgId();
		if (skipId) {
			_cornerButtons.skipReplyReturn(skipId);
		}
		...
		_jumpToMessageRequest = 0;
		setMsgId(jumpToMessageId, jumpToMessageParams);
		historyLoaded();
	} else {
		LOG(("HISTORY_LOAD: messagesReceived NO MATCH requestId=%1 "
			"_preload=%2 _preloadDown=%3 _firstLoad=%4 _jumpToMsg=%5")
			.arg(requestId)
			.arg(_preloadRequest)
			.arg(_preloadDownRequest)
...
void HistoryWidget::firstLoadMessages() {
	if (!_history || _firstLoadRequest) {
		LOG(("HISTORY_LOAD: firstLoadMessages SKIP _history=%1 _firstLoadRequest=%2")
			.arg(Logs::b(_history != nullptr))
			.arg(_firstLoadRequest));
		return;
	}

	LOG(("HISTORY_LOAD: firstLoadMessages ENTER peer=%1")
...
void HistoryWidget::sendTakeoutHistoryRequest(
		...
		int minId) {
	if (!session().api().takeoutId().has_value()) {
		LOG(("HISTORY_LOAD: sendTakeoutHistoryRequest called without takeoutId, "
			"falling back to loadTakeoutMessages"));
		loadTakeoutMessages(history, offsetId, offset, loadCount, requestSlot, maxId, minId);
		return;
	}

	LOG(("HISTORY_LOAD: sending takeout history request peer=%1 "
		"loadCount=%2 offsetId=%3").arg(history->peer->id.value)
		.arg(loadCount).arg(offsetId.bare));
	...
	requestSlot = histories.sendRequest(history, type, [=](Fn<void()> finish) {
		return session().api().request(
			MTPInvokeWithTakeout<MTPmessages_GetHistory>(
				MTP_long(*session().api().takeoutId()),
			...
			messagesReceived(history->peer, result, *requestSlotPtr);
			finish();
		}).fail([=](const MTP::Error &error) {
...
void HistoryWidget::loadMessagesDown() {
	...
	if (from->peer->isRestricted() && session().api().takeoutId().has_value()) {
		...
		if (maxMsg && !from->loadedAtBottom()) {
			const auto tloadCount = loadCount * 2;
			const auto upperBound = maxMsg + tloadCount + 1;
			loadTakeoutMessages(
				from,
				upperBound,
				0,
			...
			loadTakeoutMessages(from, offsetId + 1, addOffset, loadCount, _preloadDownRequest);
		}
		return;
	}
	const auto offsetDate = 0;
...
void HistoryWidget::jumpToMessage(
		MsgId showAtMsgId,
		const Window::SectionShow &params) {
	if (!_history) {
		return;
	}
	jumpToMessageParams = params;
	if (_jumpToMessageRequest && jumpToMessageId == showAtMsgId) {
		return;
	}

	clearAllLoadRequests();
	jumpToMessageId = showAtMsgId;

	auto from = _history;
	auto offsetId = MsgId();
	auto offset = 0;
	auto loadCount = kMessagesPerPage;
	if (jumpToMessageId == ShowAtUnreadMsgId) {
		if (const auto around = _migrated ? _migrated->loadAroundId() : 0) {
			from = _migrated;
			offset = -loadCount / 2;
			offsetId = around;
	...
	} else if (jumpToMessageId == ShowAtTheEndMsgId) {
		loadCount = kMessagesPerPageFirst;
	} else if (jumpToMessageId > 0) {
		offset = -loadCount / 2;
		offsetId = jumpToMessageId;
	} else if (jumpToMessageId < 0 && _history->peer->isChannel()) {
		if ((jumpToMessageId < 0)
			&& (-jumpToMessageId < ServerMaxMsgId)
			&& _migrated) {
			from = _migrated;
			offset = -loadCount / 2;
			offsetId = -jumpToMessageId;
	...
	const auto type = Data::Histories::RequestType::History;
	auto &histories = history->owner().histories();
	_jumpToMessageRequest = histories.sendRequest(history, type, [=](
			Fn<void()> finish) {
		return history->session().api().request(MTPmessages_GetHistory(
			history->peer->input(),
			MTP_int(offsetId),
			...
			messagesReceived(history->peer, result, _jumpToMessageRequest);
			finish();
		}).fail([=](const MTP::Error &error) {
			messagesFailed(error, _jumpToMessageRequest);
			finish();
		}).send();
	});
}
